...
export class SpatialGrid {
  ...
  // Visit every animal matching `accept` within `radius` (excluding `self`).
  // Deterministic iteration order (bucket order), so it's snapshot-safe.
  forEachNear(x: number, y: number, radius: number, accept: (a: Animal) => boolean, self: Animal | undefined, visit: (a: Animal, dist2: number) => void): void {
